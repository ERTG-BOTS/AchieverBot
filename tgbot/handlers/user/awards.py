...
from tgbot.keyboards.user.awards import (
    AwardsMenu,
    awards_kb,
    AvailableAwardsMenu,
    awards_available_kb,
...
async def awards_available_page_handler(
    callback: CallbackQuery, callback_data: AvailableAwardsMenu, achiever_db
):
    """
...
async def award_select_handler(
    callback: CallbackQuery, callback_data: AwardSelect, achiever_db, stp_db
):
    """
    ...
    async with achiever_db() as session:
        ...
        available_awards = await repo.awards.get_available_awards(
            user_id=callback.from_user.id, user_balance=user_balance
        )

    # Валидация выбора награды
    selected_award = next(
        (award for award in available_awards if award.Id == callback_data.award_id),
        None,
    )
    ...
    if selected_award.Sum > user_balance:
        await callback.answer(
            f"❌ Недостаточно очков! Нужно: {selected_award.Sum}, у вас: {user_balance}"
        )
        return

    async with achiever_db() as achiever_session:
        repo = RequestsRepo(achiever_session)
        db_selected_award: Awards = await repo.awards.get_award(
    ...
    if db_selected_award.IsShiftDependent:
        async with stp_db() as stp_session:
            ...
            is_working = await repo.buffer.is_user_working_today(
                fio=user.FIO, division=user.Division
            )

        if not is_working:
            await callback.answer(
                "❌ Награда доступна только в основные рабочие\доп смены!"
            )
            return

    await callback.message.edit_text(
        f"""<b>👏 Активация награды</b>
...
async def confirm_award(
    callback: CallbackQuery, callback_data: AwardsMenu, state: FSMContext, achiever_db
):
    await callback.answer()
    async with achiever_db() as achiever_session:
        repo = RequestsRepo(achiever_session)
...
async def comment_award(message: Message, state: FSMContext, achiever_db, stp_db):
    ...
    execute = await repo.executes.add_pending_award(
        award=award, user=user, comment=message.text
    )
    ...
    await message.bot.edit_message_text(
        ...
        text=f"""<b>✅️ Награда приобретена</b>
...
async def show_available_awards_page(
    callback: CallbackQuery, achiever_db, page: int = 1
):
    """
    ...
    total_pages = (
        (total_awards + awards_per_page - 1) // awards_per_page
        if total_awards > 0
        else 1
    )
...
{"\n".join(awards_list)}"""
    ...
    await callback.message.edit_text(
        message_text, reply_markup=awards_available_kb(page_awards, page, total_pages)
    )
    logging.info(
